...
C NCLFORTSTART
      subroutine remap_z_dbl(imt, jmt, klev_in, klev_out, tlev, KMT,
     +                       z_edge, VAR_IN, NEW_Z, new_z_edge, msv,
     +                       THICKNESS, VAR_OUT)
      integer imt, jmt, klev_in, klev_out, tlev
      integer KMT(imt,jmt)
      double precision msv
      double precision z_edge(*)
      double precision VAR_IN(imt,jmt,klev_in,tlev)
      double precision NEW_Z(imt,jmt,klev_in,tlev)
      double precision new_z_edge(*)
      double precision THICKNESS(imt,jmt,klev_out,tlev)
      double precision VAR_OUT(imt,jmt,klev_out,tlev)
C NCLEND
...
Cf2py intent(out) VAR_OUT
      ...
      double precision, allocatable :: dz(:), dzw(:), zt(:),
     +                                 dNEW_Z(:), dVAR_IN(:)
      double precision h, hlo, hhi, hmid
      ...
      do kin = 1, klev_in
        ...
        if (kin .gt. 1) dzw(kin) = 0.5d0*(dz(kin) + dz(kin-1))
        zt(kin) = 0.5d0*(z_edge(kin+1) + z_edge(kin))
      end do
...
C
          ...
          do kout = 1, klev_out
            THICKNESS(i,j,kout,l) = 0.0d0
            VAR_OUT(i,j,kout,l) = 0.0d0
C
...
C               NEW_Z(zt(kin)-h) = NEW_Z(i,j,kin,l) - h / dzw(kin) * dNEW_Z(kin)
     ...
     +                   dzw(kin) / dNEW_Z(kin)
                hlo = min(max(hlo,0.0d0),0.5d0*dz(kin))
                hhi = (NEW_Z(i,j,kin,l) - new_z_edge(kout+1)) *
     +                   dzw(kin) / dNEW_Z(kin)
                hhi = min(max(hhi,0.0d0),0.5d0*dz(kin))
                h = abs(hhi-hlo)
                THICKNESS(i,j,kout,l) = THICKNESS(i,j,kout,l) + h
...
C               evaluate this at hmid
     ...
     +                                h * VAR_IN(i,j,kin,l)
            ...
            if (THICKNESS(i,j,kout,l) .gt. 0.0d0) then
              VAR_OUT(i,j,kout,l) = VAR_OUT(i,j,kout,l) /
